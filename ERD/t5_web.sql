...
(
  ...
  redgate DATETIME NULL     DEFAULT now(),
  PRIMARY KEY (id)
) COMMENT '댓글';
...
ALTER TABLE t5_user_authorities
  ADD CONSTRAINT FK_t5_user_TO_t5_user_authorities
    ...
    ON UPDATE RESTRICT
    ON DELETE CASCADE   # 삭제 되었을 때 자동으로 삭제
;
...
ALTER TABLE t5_post
  ADD CONSTRAINT FK_t5_user_TO_t5_post
    ...
    REFERENCES t5_user (id)
    ON UPDATE RESTRICT
    ON DELETE CASCADE   # 삭제 되었을 때 자동으로 삭제 (사용자가 탈퇴하면 글 삭제)
;
...
ALTER TABLE t5_comment
  ADD CONSTRAINT FK_t5_user_TO_t5_comment
    ...
    REFERENCES t5_user (id)
    ON UPDATE RESTRICT
    ON DELETE CASCADE   # 삭제 되었을 때 자동으로 삭제  (사용자가 탈퇴하면 댓글 삭제)
;
...
ALTER TABLE t5_comment
  ADD CONSTRAINT FK_t5_post_TO_t5_comment
    ...
    ON UPDATE RESTRICT
    ON DELETE CASCADE   # 삭제 되었을 때 자동으로 삭제 (글이 삭제되면 댓글 삭제)
;
...
ALTER TABLE t5_attachment
  ADD CONSTRAINT FK_t5_post_TO_t5_attachment
    ...
    REFERENCES t5_post (id)
    ON UPDATE RESTRICT
    ON DELETE CASCADE   # 삭제 되었을 때 자동으로 삭제 (글 삭제되면 첨부파일 삭제)
;
